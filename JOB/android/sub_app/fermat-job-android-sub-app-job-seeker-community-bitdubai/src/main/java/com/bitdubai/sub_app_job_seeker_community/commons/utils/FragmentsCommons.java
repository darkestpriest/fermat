package com.bitdubai.sub_app_job_seeker_community.commons.utils;
...
public class FragmentsCommons {
    ...
    View view = inflater.inflate(
            R.layout.jjsc_row_navigation_drawer_community_header,
            relativeLayout,
            true);
    ...
    if (identity != null) {
        if (identity.getImage() != null) {
            if (identity.getImage().length > 0) {
                ...
                Picasso.with(activity).load(R.drawable.jjsc_profile_image).into(imageView);
        } else
            Picasso.with(activity).load(R.drawable.jjsc_profile_image).into(imageView);
